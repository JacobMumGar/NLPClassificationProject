...
def TFIDF(train, test):
    #convert the text to TF-IDF feature vectors using sklearn package, stop words are removed
...
def sentenceTransform(train, test):
    #load the pretrained model and encode the train and test features
    model = SentenceTransformer("all-MiniLM-L6-v2")
    trainEmbeddings = model.encode(list(train), batch_size=256, show_progress_bar=True)
    testEmbeddings = model.encode(list(test), batch_size=256, show_progress_bar=True)
